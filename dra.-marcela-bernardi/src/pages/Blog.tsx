...
const Blog = () => {
  ...
  useEffect(() => {
    ...
    if (metaDescription) {
      metaDescription.setAttribute("content", "Acompanhe nosso blog para dicas de saúde feminina, obstetrícia humanizada e ginecologia integrativa com a Dra. Marcela Bernardi.");
    }
  }, []);

  const categories = useMemo(() => {
    const cats = blogPosts.map(post => post.categoria);
                  ...
                  {recentPosts.map(post => (
                    <Link key={post.id} to={`/blog/${post.id}`} className="flex gap-4 group">
                      <div className="w-20 h-20 rounded-xl overflow-hidden flex-shrink-0">
                        <img src={post.imagem} alt={post.titulo} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" referrerPolicy="no-referrer" />
                      </div>
                      <div className="flex flex-col justify-center">
                        <h4 className="text-sm font-medium text-[#4A5550] line-clamp-2 group-hover:text-[#82B596] transition-colors leading-snug">
                          {post.titulo}
                        </h4>
                        <span className="text-[10px] text-gray-400 mt-1 uppercase tracking-wider">{post.data}</span>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            <AnimatePresence mode="wait">
              {filteredPosts.length > 0 ? (
                ...
                >
                  ...
                  <h3 className="text-xl font-medium text-[#4A5550] mb-2">Nenhuma postagem encontrada</h3>
                  <p className="text-gray-400 font-light">Tente buscar por outros termos ou mudar a categoria.</p>
                  <button 
                    onClick={() => { setSelectedCategory(null); setSearchQuery(''); }}
                    className="mt-8 text-[#82B596] font-medium hover:underline"
                  >
